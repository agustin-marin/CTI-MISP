...
import models.differentialprivacy.ResponseAttElement;
import models.differentialprivacy.ResponseAttScript;
import models.differentialprivacy.ResponseObjScript;
...
public class PrivacyOperations {
	...
	public void apply_dp_to_object_in_event(Event e, String name_object, Att_agrupation ag, boolean apply_all, String pythonScript) {
		//get all possible attributes in this event in this type of object
		ArrayList<String> att_names = new ArrayList<>();
		if(apply_all) {
			for(Object o : e.getObject()) {
				if(o.getName().equals(name_object)) {
					for(Attribute a : o.getAttribute()) {
						if(!att_names.contains(a.getObject_relation())) att_names.add(a.getObject_relation());
					}
				}
			}
		}else {
			att_names = (ArrayList<String>)ag.getAtt_names();
			for(String a : att_names) System.out.println(a);
		}
		
		
		//uuidobject + value in index correspondant to att_names
		HashMap<String, String[]> att_values = new HashMap<>();
		for(Object o : e.getObject()) {
			if(o.getName().equals(name_object)) {
				...
				//check what happens when a.getValue() does not have value
				for(Attribute a : o.getAttribute()) {
					if(att_names.contains(a.getObject_relation())) {
						values[att_names.indexOf(a.getObject_relation())] = a.getValue();
					}
				}
				att_values.put(o.getUuid(), values);
			}
		}
		//att_values at this point has pair uuid + [valueatt1,...,valueattn]
		//creating json with format specified in dpRelated dir
		JSONObject json = new JSONObject();
		//JSONArray names = new JSONArray();
		//names.put(att_names);
		json.put("att_columns", att_names);
		JSONArray objects = new JSONArray();
		for(String uuid : att_values.keySet()) {
			JSONObject object = new JSONObject();
			object.put("uuid", uuid);
			object.put("values", att_values.get(uuid));
			objects.put(object);
		}
		json.put("objects", objects);
		json.put("epsilon", ag.getMetadata().getEpsilon());
		json.put("delta", ag.getMetadata().getDelta());
		json.put("sensitivity", ag.getMetadata().getSensitivity());
		json.put("mechanism", ag.getScheme());
		//tells the python script type of dp to apply, attributes or objects
		json.put("dp_in_objects", true);
		json.put("dp_in_attributes", false);
		//
		if(ag.getMetadata().getAtt_low_bounds() != null) {
			System.out.println("not null");
			json.put("bottom", ag.getMetadata().getAtt_low_bounds());
		}
		if(ag.getMetadata().getAtt_high_bounds() != null) {
			System.out.println("not null");
			json.put("top", ag.getMetadata().getAtt_high_bounds());
		}
		System.out.println("Json: " + json.toString());
		//json is the input for python script
		//python script returns values in order as they where in input json, but modified.
		Process process = null;
		//TODO: put scriptname in directory of execution. Will need to install python dependencies.
		//String scriptName = "/home/juanfran/Documentos/ProducerScripts/dpRelated/applyDp.py";
		String scriptName = pythonScript + "applyDp.py";
		//Example of what this script will return, array of array
		//{ "values": [[1.5702891834472574, 'null'], [0.4408163974305148, 'null']]}
		System.out.println(json.toString());
		String response = null;
		StringBuilder sb = new StringBuilder();
		Writer bufferdp = null;
		String inputnamegenerated = this.requestbufferdp + java.util.UUID.randomUUID().toString();
		String outputnamegenerated = this.responsebufferdp + java.util.UUID.randomUUID().toString();
		File inputfile = null;
		File outputfile = null;
		try {
			inputfile = new File(inputnamegenerated);
			inputfile.createNewFile();
			outputfile = new File(outputnamegenerated);
			outputfile.createNewFile();
			bufferdp = new BufferedWriter(new FileWriter(inputnamegenerated));
			bufferdp.write(json.toString());
			bufferdp.close();
		}catch (Exception excep){
			excep.printStackTrace();
			return;
		}
		ProcessBuilder pb = new ProcessBuilder(new String[] {"python3", scriptName, "-i", inputnamegenerated, "-o", outputnamegenerated});
		pb.redirectErrorStream(true);
		try {
			String request = json.toString();
			JSONObject rq = new JSONObject(request);
			//String args = "python3 " + scriptName + " -v " + rq.toString() + "";
		...
		System.out.println("Response python script " + sb.toString());
		/*if(!sb.toString().equals("OK")){
			comprobar error o funcionamiento correcto del script
		}*/
		//read response file
		Scanner scanner_rf = null;
		try {
			scanner_rf = new Scanner(outputfile);
		} catch (FileNotFoundException ex) {
			throw new RuntimeException(ex);
		}
		StringBuilder response_file_buffer = new StringBuilder("");
		while(scanner_rf.hasNextLine()){
			response_file_buffer.append(scanner_rf.nextLine());
		}
		scanner_rf.close();
		System.out.println("Response file buffer " + response_file_buffer.toString());
		ResponseObjScript rs = new Gson().fromJson(response_file_buffer.toString(), ResponseObjScript.class);
		int index = 0;
		for(Object o : e.getObject()) {
			if(o.getName().equals(name_object)) {
				//check uuids in final event
				int index_att = 0;
				for(String name : att_names) {
					if(rs.getValues().get(index).get(index_att) != "null") {
						o.setAttributeValue(name, rs.getValues().get(index).get(index_att));
					}
				index_att++;
				}
			}
	...
	public void apply_dp_to_attributes_in_event(Event event, PrivacyPolicy pol) {
		//get att for which apply dp
		ArrayList<String> att_names = new ArrayList<>();
		for(AttPolicy ap : pol.getAttributes()) {
			if(!att_names.contains(ap.getName()) && ap.isDp()) {
				att_names.add(ap.getName());
				System.out.println("Added " + ap.getName());
			} 
		}
		
		HashMap<String, ArrayList<String>> att_values = new HashMap<>();
		for(Attribute a : event.getAttribute()) {
			String attname = a.getObject_relation();
			System.out.println("Name att " + attname);
			if(att_names.contains(attname)) {
				if(!att_values.containsKey(attname)) att_values.put(attname, new ArrayList<>());
				att_values.get(attname).add(a.getValue());
				System.out.println("Added value " + a.getValue());
			}
		}
		JSONObject json = new JSONObject();
		JSONArray attributes = new JSONArray();
		for(String att : att_values.keySet()) {
			JSONObject j_att = new JSONObject();
			j_att.put("att_name", att);
			Att_dp_param dp_params = pol.getAtt(att).getDp_params();
			j_att.put("mechanism", dp_params.getScheme());
			j_att.put("epsilon", dp_params.getMetadata().getEpsilon());
			j_att.put("delta", dp_params.getMetadata().getDelta());
			j_att.put("sensitivity", new BigDecimal(dp_params.getMetadata().getSensitivity()).toPlainString());
			//j_att.put("sensitivity", Double.valueOf(String.format(Locale.US,"%.12f", dp_params.getMetadata().getSensitivity())));
			if(dp_params.getMetadata().getAtt_low_bounds() != null) j_att.put("bottom", dp_params.getMetadata().getAtt_low_bounds());
			if(dp_params.getMetadata().getAtt_high_bounds() != null) j_att.put("top", dp_params.getMetadata().getAtt_high_bounds());
			j_att.put("values", att_values.get(att));
			attributes.put(j_att);
		}
		json.put("dp_in_objects", false);
		json.put("dp_in_attributes", true);
		...
		StringBuilder sb = new StringBuilder();
		System.out.println("Json : " + json.toString());
		//write request data in input buffer
		//{} write and read to basename file + uuid, when process finished and output file read, remove file
		Writer bufferdp = null;
		String inputnamegenerated = this.requestbufferdp + java.util.UUID.randomUUID().toString();
		String outputnamegenerated = this.responsebufferdp + java.util.UUID.randomUUID().toString();
		File inputfile = null;
		...
		ProcessBuilder pb = new ProcessBuilder(new String[] {"python3", scriptLocation,"-i", inputnamegenerated , "-o", outputnamegenerated});
		pb.redirectErrorStream(true);
		try {
		...
		//TODO: read response from output buffer
		Scanner scanner_rf = null;
		try {
			scanner_rf = new Scanner(outputfile);
		...
		scanner_rf.close();
		//TODO: add comprobation to check if response from script is error
		System.out.println("Response python script " + sb.toString());
		ResponseAttScript rs = new Gson().fromJson(response_file_buffer.toString(), ResponseAttScript.class);
		
		HashMap<String, Integer> att_n = new HashMap<>();
		for(String att : att_values.keySet()) {
			att_n.put(att, 0);
		}
		...
		for(Attribute a : event.getAttribute()) {
			String attname = a.getObject_relation();
			if(att_values.keySet().contains(attname)) {
				a.setValue(rs.getSpecificValue(attname, att_n.get(attname)));
				att_n.put(attname, att_n.get(attname) + 1);
			}
		}

		//delete temporal files if them were created
		if(inputfile != null){
			inputfile.delete();
		}
		if(outputfile != null){
	...
	//TODO: CONTINUACION
	//objects_or_attribute: true attributes pol, false objects pol
	public void check_dp_in_policy(Event event, PrivacyPolicy privacypolicy, boolean objects_or_attributes, String pythonScript) {
		
		if(objects_or_attributes) {
			apply_dp_to_attributes_in_event(event, privacypolicy);
		}else {
			for(Template t : privacypolicy.getTemplates()) {
				if(t.getAtt_aggroupations() != null) {
//					for(Att_agrupation ag : t.getAtt_aggroupations()) {
//						
//					}
					//TODO: now supponsing that only one differential privacy for object. This could change.
					Att_agrupation ag = t.getAtt_aggroupations().get(0);
					boolean applytoall = ag.isApply_to_all();
					apply_dp_to_object_in_event(event, t.getName(), ag, applytoall, pythonScript);
					//in event dp has been applied, its modified
				}
			}
		}
	}
	...
	//applies differential privacy to a event
	public Event applydifferentialprivacy(Event event, PrivacyPolicy pol, boolean type, String pythonScript) {
		check_dp_in_policy(event, pol, type, pythonScript);
		return event;
	}
